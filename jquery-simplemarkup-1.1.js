...
var simple_markup = function(options, textarea){
	var that = {}, wrap, canvas, toolbar, status, is_blocked = false, ie_sel_buffer = {};
	...
	var get_sel = function(){
		var selection, caretPosition, selectionEnd,
		...
		}else if($.browser.opera && $.browser.version > 9.64){ // opera * > opera 9.64
			
			val = dom.value;
			
			caretPosition = dom.selectionStart;
			selectionEnd = dom.selectionEnd;
			selection = val.substring(caretPosition, selectionEnd);
		...
		return ie_sel_buffer;
	},
	set_sel = function (start, length, adj_for_opera) {
		var dom = textarea.focus().get(0), value = dom.value;
		
		if (arguments.length < 3 || arguments[2] == true) {
			if ($.browser.opera && value.indexOf('\r') !== -1) { // opera on windows
				var i = 0;
				while (i <= start) {
					if (value.charAt(i) === '\r') {
						start++;
...
//			replaceWith: '',
		...
		for(i = 0; i < options.markupSet.length; i++){
			...
			buttons.append($('<li class="smu-button ui-state-default ui-corner-all '+button.className+'"><div class="smu-button-icon '+button.iconClass+'"></div><div class="smu-button-label"'+(button.label.length ? '' : ' style="diaplay: none;"')+'>'+button.label+'</div></li>')
	...
	markup = function(setting, event){
		status = $.extend({}, setting, $.browser.msie ? ie_sel_buffer : get_sel());
		status.event = event;
		...
		set_sel( status.end + (before.length - status.before.length)
			...
			+ (after.length - status.after.length) , 0, false);
	},
